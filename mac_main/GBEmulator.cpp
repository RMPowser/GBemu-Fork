...
GBEmulator::GBEmulator(const char* rom_filename)
  ...
  // Initialize macOS UI with the SDL window
  mac_ui_.initialize(window_.get_sdl_window(), window_.get_max_scale_factor());

  // Set up macOS UI callbacks
  mac_ui_.set_on_open_rom([this](const std::string& path) { this->open_rom(path); });
  mac_ui_.set_on_restart_gameboy([this]() {
    if (!current_rom_path_.empty()) {
      open_rom(current_rom_path_);
    }
  });
  mac_ui_.set_on_save([this](const std::string& path) { this->save(path); });
  mac_ui_.set_on_quick_save([this]() { this->quick_save(); });
  mac_ui_.set_on_quick_load([this]() { this->quick_load(); });
  mac_ui_.set_on_select_boot_rom([this](const std::string& path) { this->set_boot_rom(path); });
  mac_ui_.set_on_scale_change([this](uint32_t factor) { window_.apply_scale_factor(factor); });
  mac_ui_.set_on_prepare_pause([this]() { window_.prepare_for_pause(); });
  mac_ui_.set_on_resume_pause([this]() { window_.resume_from_pause(); });
  mac_ui_.set_on_exit([]() { std::exit(0); });
...
void GBEmulator::show_error(const std::string& title, const std::string& message) {
  mac_ui_.show_error(window_.get_sdl_window(), title, message);
}
